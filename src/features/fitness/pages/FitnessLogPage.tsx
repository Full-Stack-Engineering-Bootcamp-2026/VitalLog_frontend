...
import type { FitnessLogResponseDto, FitnessFilters, PaginatedFitnessResponse } from "../types/fitness.types"
...
export default function FitnessLogPage() {
  ...
  const loadLogs = useCallback(async () => {
    try {
      setLoading(true)
      const [tableData, chartData, streakData]: [PaginatedFitnessResponse, PaginatedFitnessResponse, StreakResponseDto] =
        await Promise.all([
  ...
  const totalCalories = allLogs.reduce((sum, log) => sum + log.caloriesBurned, 0)
  ...
  const columns = [
    ...
    },
    {
      header: "Notes",
      accessor: (row: FitnessLogResponseDto) => (
        <span className="italic text-gray-400">{row.notes ?? "—"}</span>
      ),
    },
    {
      header: "Actions",
      accessor: (row: FitnessLogResponseDto) => (
        <div className="flex items-center gap-2">
          <button onClick={() => navigate(`/fitness-log/edit/${row.id}`)} className="text-gray-400 hover:text-gray-700 transition-colors">
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        action={
          <Button onClick={() => navigate("/fitness-log/log")} className="bg-green-600 hover:bg-green-700 text-white">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="col-span-2 rounded-xl bg-white p-5 shadow-sm">
          <p className="text-sm font-semibold text-gray-700">Activity Frequency</p>
          <p className="mb-3 text-xs text-gray-400">Last 30 days distribution</p>
        ...
        <div className="flex flex-col gap-4">
          <StatCard
            ...
            subtext={streak.longestStreak > 0 ? `Best: ${streak.longestStreak} days` : "Keep it up!"}
            color="amber" />
          <StatCard
      ...
      <div className="rounded-xl bg-white p-5 shadow-sm">
        <div className="mb-4 flex items-center gap-3">
          <span className="text-xs font-medium uppercase tracking-wide text-gray-400">Activity</span>
          <Select defaultValue="ALL" onValueChange={(val) => setFilters((prev) => ({
            ...prev,
            page: 1,
        ...
        {!loading && logs.length > 0 && (
          <div className="mt-4 flex items-center justify-between border-t border-gray-100 pt-4">
            <p className="text-sm text-gray-400">Showing {logs.length} of {total} entries</p>
            <Pagination
              page={filters.page ?? 1}
